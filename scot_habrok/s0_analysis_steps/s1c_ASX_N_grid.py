...
for sub in sub_list:
    ...
    for task in task_list:    
        ...
        script_args = f"--sub {sub} --task {task} --roi_fit {roi_fit} --n_jobs {n_jobs} {constraint} --prf_out {prf_out} --grid_only --model {model}"
        os.system(f'{job} {slurm_args} {script_path} --job-name {prf_job_name} --output-dir {this_dir} --args "{script_args}"')
        # sys.exit()
