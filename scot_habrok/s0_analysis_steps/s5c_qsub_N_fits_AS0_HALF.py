...
sub_list = ['sub-04'] # , 'sub-02', 'sub-03', 'sub-04', 'sub-05', 'sub-06']
roi_fit = 'all'
constraint = '--tc'
nr_jobs = 15
...
for sub in sub_list:
    ...
    for task in task_list:
        prf_job_name = f'Nhalf-{task}-{sub}-{roi_fit}'            
        ...
        script_args = f"--sub {sub} --task {task} --model norm --roi_fit {roi_fit} --nr_jobs {nr_jobs} {constraint} --prf_out {prf_out}"
